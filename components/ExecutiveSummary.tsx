export default function ExecutiveSummary() {
  return (
    <section className="mb-10">
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="bg-brand-700 px-6 py-4">
          <h2 className="text-lg font-bold text-white">Executive Summary</h2>
        </div>

        <div className="px-6 py-6 text-gray-800 space-y-4 text-sm leading-relaxed">

          {/* Intro paragraphs */}
          <p>
            Public trust is the moral foundation of the organ donation and transplantation
            system. Unlike most areas of medicine, organ donation depends on voluntary altruism
            at moments of profound vulnerability, when patients and families must rely on
            institutions to act ethically, competently, and transparently in circumstances they
            cannot independently verify. Recent investigative reporting, regulatory scrutiny,
            and public concern have exposed both real failures and perceived risks within the
            system, threatening this trust and prompting widespread donor hesitation and
            deregistration.
          </p>

          <p>
            This work proposes a Donor Bill of Rights as a unifying ethical framework designed
            to restore and sustain public trust by making explicit the rights of patients,
            donors, families, and the public—and the corresponding obligations of healthcare
            institutions, professional societies, organ procurement organizations, and governing
            bodies. Drawing on peer-reviewed literature, established ethical theory, legal
            precedent, qualitative research on family experience, and contemporary policy
            analysis, the Bill articulates fifteen interdependent rights spanning end-of-life
            care, death determination, consent, equity, communication, oversight, transparency,
            and ethical stewardship.
          </p>

          <p>
            Rather than introducing new ethical principles, this framework synthesizes existing
            norms that are often fragmented across disciplines and institutions, translating
            them into a coherent, patient-centered structure that can be publicly affirmed,
            operationalized, and evaluated. Its central claim is simple but consequential: organ
            donation is ethically legitimate only when it is visibly downstream of high-quality,
            value-concordant end-of-life care, governed by independent safeguards, and embedded
            within a system that learns, reforms, and earns trust over time.
          </p>

          {/* Integrated Synthesis */}
          <h3 className="text-base font-bold text-brand-800 pt-2">
            Integrated Synthesis: A Rights-Based Architecture for Trustworthy Organ Donation
          </h3>

          {/* Section 1 */}
          <h4 className="font-semibold text-gray-900">1. Trust as the Organizing Principle</h4>
          <p>
            Across all fifteen articles, trust functions not as a rhetorical goal but as the
            organizing ethical principle. Trust is neither blind faith nor passive confidence;
            it is a reasoned judgment that institutions are worthy of reliance. In the context
            of organ donation, trust must be earned through structure, not merely asserted
            through intention.
          </p>
          <p>
            The Bill of Rights treats trust as a public good—fragile, cumulative, and
            system-level—rather than as an interpersonal sentiment confined to the bedside.
            This framing explains why rare but high-profile failures can destabilize the entire
            system, and why reforms must be structural, visible, and enforceable rather than
            reactive or performative.
          </p>

          {/* Section 2 */}
          <h4 className="font-semibold text-gray-900">2. Primacy of End-of-Life Care and Moral Pluralism</h4>
          <p>Articles 1–4 establish the ethical boundary conditions of legitimate organ donation:</p>
          <ul className="list-disc pl-5 space-y-1">
            <li>End-of-life care must be independent, patient-centered, and uncoupled from procurement.</li>
            <li>Decisions must honor pluralistic value frameworks, including &ldquo;sanctity-of-life&rdquo;, &ldquo;sanctity-of-function&rdquo; orientations, and everything in between.</li>
            <li>Death must be determined using rigorous, transparent, evidence-based standards, separate from donation considerations.</li>
          </ul>
          <p>
            Together, these articles reject the assumption—sometimes implicit in public
            discourse—that ethical donation requires a &ldquo;life-at-all-costs&rdquo; posture. Instead,
            they affirm that donation is ethically permissible only when it follows, rather
            than drives, values-based clinical decisions. This reframing is critical for
            restoring trust among families who fear that choosing comfort, dignity, or
            functional thresholds places them at moral or medical disadvantage.
          </p>

          {/* Section 3 */}
          <h4 className="font-semibold text-gray-900">3. Protection of Autonomy Under Conditions of Vulnerability</h4>
          <p>
            Articles 5, 6, and 13 address a core ethical tension: how to preserve voluntariness
            when decisions are made under emotional distress, time pressure, and informational
            asymmetry.
          </p>
          <p>The Bill explicitly recognizes that coercion is often subtle, unintentional, and systemic rather than malicious. By affirming rights to:</p>
          <ul className="list-disc pl-5 space-y-1">
            <li>protection from coercive framing and misinformation,</li>
            <li>access to independent advocacy and ethical oversight, and</li>
            <li>informed, revocable, and respected consent,</li>
          </ul>
          <p>
            the framework shifts ethical responsibility from individual conversations to
            institutional design. Consent is treated not as a transactional checkbox, but as a
            relational and revisable process, supported by safeguards that protect families even
            when they are uncertain, conflicted, or ultimately decline donation.
          </p>

          {/* Section 4 */}
          <h4 className="font-semibold text-gray-900">4. Communication, Dignity, and the Human Experience of Loss</h4>
          <p>
            Articles 7 and 14 address the lived experience of families before, during, and
            after death. They underscore that ethical legitimacy depends on what decisions are
            made, and how they are communicated and embodied.
          </p>
          <p>
            High-quality communication, emotional support, bereavement care, and respectful
            treatment of the body are framed as ethical obligations rather than ancillary
            services. This emphasis reflects empirical evidence that families&apos; long-term trust
            in healthcare systems is shaped less by outcomes than by whether they felt heard,
            respected, and protected at moments of irreversible loss.
          </p>
          <p>
            By explicitly extending dignity beyond death, the Bill counters one of the most
            persistent public fears about organ donation: that bodies will be treated
            instrumentally rather than reverently.
          </p>

          {/* Section 5 */}
          <h4 className="font-semibold text-gray-900">5. Equity, Transparency, and Accountability as Trust Conditions</h4>
          <p>
            Articles 8 and 9 confront systemic inequities and governance failures that undermine
            trust even among those who never encounter donation directly.
          </p>
          <p>
            The Bill rejects the idea that equity and transparency are aspirational goals.
            Instead, it treats them as preconditions for moral legitimacy. A system that
            solicits altruism while delivering inequitable access to transplantation, opaque
            performance data, or weak accountability cannot reasonably expect public trust.
          </p>
          <p>
            By embedding equity measurement, public reporting, safety event disclosure, and
            enforceable accountability into the rights framework, the Bill aligns ethical theory
            with contemporary expectations of institutional responsibility.
          </p>

          {/* Section 6 */}
          <h4 className="font-semibold text-gray-900">6. Public Discourse, Participation, and Adaptive Ethics</h4>
          <p>
            Articles 10 and 11 recognize that trust is shaped not only by clinical practice but
            by public narrative and governance culture.
          </p>
          <p>
            Responsible media engagement, proactive correction of misinformation, and
            transparent acknowledgment of failure are framed as ethical duties rather than
            reputational strategies. Likewise, stakeholder participation and ongoing ethical
            deliberation are positioned as mechanisms for moral adaptation in a rapidly evolving
            clinical and technological landscape.
          </p>
          <p>
            This emphasis reflects a core insight of the Bill: ethical systems that do not
            listen will eventually lose legitimacy, regardless of technical sophistication.
          </p>

          {/* Section 7 */}
          <h4 className="font-semibold text-gray-900">7. Learning, Reform, and Ethical Stewardship</h4>
          <p>
            Articles 12 and 15 bind the framework together by asserting that trustworthiness is
            sustained through continuous learning and ethical stewardship.
          </p>
          <p>Rather than promising perfection, the Bill commits institutions to:</p>
          <ul className="list-disc pl-5 space-y-1">
            <li>detect and analyze failures and near misses,</li>
            <li>reform practices transparently, and</li>
            <li>place public interest above institutional self-protection.</li>
          </ul>
          <p>
            Ethical stewardship is framed as a collective responsibility that extends beyond
            compliance to include moral leadership, humility, and willingness to change. In this
            sense, the Bill of Rights is not static; it is designed to remain ethically
            responsive as medicine, technology, and societal values evolve.
          </p>

          {/* Concluding Synthesis */}
          <h3 className="text-base font-bold text-brand-800 pt-2">Concluding Synthesis</h3>
          <p>
            Taken together, the fifteen articles form a coherent, rights-based architecture for
            ethical organ donation grounded in trust, dignity, autonomy, equity, and
            accountability. The framework does not seek to undermine organ donation; rather, it
            aims to protect it from erosion caused by silence, fragmentation, and misplaced
            defensiveness.
          </p>
          <p>
            By making explicit what patients and families should expect—and what institutions
            must guarantee—the Donor Bill of Rights offers a path toward restoring public
            confidence at a moment of legitimate concern and scrutiny. A system that earns trust
            does more than facilitate transplantation; it honors the moral weight of the gift on
            which it depends.
          </p>

        </div>
      </div>
    </section>
  );
}
